"""
Unified Visualization: Flood + I/J Points + BTS Active / Power Outage / Failed / COW / Recovered BTS
Author: Lợi Lưu – 2025
"""
...
# MILP outputs — NEW
COW_ASSIGN = PROJECT_ROOT / "outputs/milp_runs/milp_gurobi/assignments_cow_GUROBI.csv"
POWER_ASSIGN = PROJECT_ROOT / "outputs/milp_runs/milp_gurobi/assignments_power_GUROBI.csv"

# Output
OUTPUT_DIR = PROJECT_ROOT / "outputs/visualization"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
HTML_OUTPUT = OUTPUT_DIR / "visualization_combined_milp.html"
...
def load_bts():
    ...
    gdf_active = gpd.GeoDataFrame(df_active,
        geometry=gpd.points_from_xy(df_active.longitude, df_active.latitude),
        crs="EPSG:4326")

    gdf_failed = gpd.GeoDataFrame(df_failed_failed,
        geometry=gpd.points_from_xy(df_failed_failed.longitude, df_failed_failed.latitude),
        crs="EPSG:4326")

    gdf_power = gpd.GeoDataFrame(df_failed_power,
        geometry=gpd.points_from_xy(df_failed_power.longitude, df_failed_power.latitude),
        crs="EPSG:4326")
...
def build_bts_popup(row):
    status_text = {
        ...
        "recovered": "<span style='color:green;'>KHÔI PHỤC BẰNG NGUỒN DỰ PHÒNG</span>",
...
def add_cow_layer(map_obj, df_cow, gdf_J):
    fg = folium.FeatureGroup(name="COW Deployment", show=False)

    for _, row in df_cow.iterrows():
        j_site = row["site_id"]

        match = gdf_J[gdf_J["site_id"] == j_site]
        if match.empty:
            print(f"[WARNING] J-site {j_site} not found.")
            continue
        ...
        popup = folium.Popup(
            ...
            f"Triển khai tại: {j_site}<br>"
            f"Bán kính phủ sóng: {radius} m", max_width=350
        )
...
# ============================================================
# ADD RECOVERED BTS (NEW)
# ============================================================
def add_recovered_bts(map_obj, df_power, gdf_failed):
    fg = folium.FeatureGroup(name="Recovered BTS", show=False)
...
def run_visualization_combined():
    print("\n=== Combined Flood + I/J + BTS + COW + Recovered Visualization ===")
    ...
    # J Sites
    fg_Jlayer = folium.FeatureGroup(name="J Sites (Candidate Locations)", show=False)
