#!/usr/bin/env python3
"""
compute_population_coverage_gapso.py

Compute population coverage summary using GA-PSO assignment outputs.
Inputs (GA-PSO):
 - outputs/results_ga_pso/solution_cow_assignments.csv
 - outputs/results_ga_pso/solution_power_assignments.csv

Output:
 - outputs/summary/ga_pso/*.csv, *.geojson, coverage_report_ga_pso.json
...
ASSIGN_COW_GAPSO = OUT_DIR / "results_ga_pso" / "solution_cow_assignments_new.csv"
ASSIGN_POWER_GAPSO = OUT_DIR / "results_ga_pso" / "solution_power_assignments_new.csv"
...
J_SITES = PROCESSED_DIR / "position_I_J" / "J_sites_new.csv"
...
SUMMARY_DIR = OUT_DIR / "summary" / "ga_pso_new"
...
# -------------------------
# Compute COW coverage from GA-PSO assignments
# -------------------------
def compute_cow_coverage_from_gapso(assign_path: Path, j_sites_path: Path, cows_table_path: Path,
                                    method_tag="gapso", outage_geom=None):
    raster_crs, nodata = read_raster_crs_and_nodata(POP_RASTER)
    boundary = gpd.read_file(BOUNDARY_GEOJSON)
    if boundary.crs:
        boundary = boundary.to_crs(raster_crs)
    ...
    for idx, row in assign.iterrows():
        try:
            ...
            cow_records.append({
                ...
                "cow_base_cost_vnd": base_cost,
                "deployment_time_hr": travel_time
            })
...
# -------------------------
# Compute POWER coverage from GA-PSO assignments
# -------------------------
def compute_power_coverage_from_gapso(assign_path: Path, failed_bts_path: Path,
                                      method_tag="gapso", outage_geom=None):
    raster_crs, nodata = read_raster_crs_and_nodata(POP_RASTER)
    boundary = gpd.read_file(BOUNDARY_GEOJSON)
    if boundary.crs:
...
def main_compute_all(method="GA_PSO"):
    ...
    cow_results = compute_cow_coverage_from_gapso(ASSIGN_COW_GAPSO, J_SITES, COWS, method_tag="gapso", outage_geom=outage_geom)
    power_results = compute_power_coverage_from_gapso(ASSIGN_POWER_GAPSO, FAILED_BTS, method_tag="gapso", outage_geom=outage_geom)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default="GA_PSO", help="Method key (GA_PSO)")
