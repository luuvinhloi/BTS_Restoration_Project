...
Output:
 - outputs/summary/hybrid/*.csv, *.geojson, coverage_report_hybrid.json
...
ASSIGN_COW_HYBRID = OUT_DIR / "results_hybrid" / "solution_cow_assignments_new.csv"
ASSIGN_POWER_HYBRID = OUT_DIR / "results_hybrid" / "solution_power_assignments_new.csv"
...
SUMMARY_DIR = OUT_DIR / "summary" / "hybrid_new"
...
def compute_cow_coverage_from_hybrid(assign_path: Path, j_sites_path: Path, cows_table_path: Path,
                                    method_tag="hybrid", outage_geom=None):
    raster_crs, nodata = read_raster_crs_and_nodata(POP_RASTER)
    boundary = gpd.read_file(BOUNDARY_GEOJSON)
    if boundary.crs:
        boundary = boundary.to_crs(raster_crs)
...
def main_compute_all(method="MILP_GA_PSO"):
    ...
    cow_results = compute_cow_coverage_from_hybrid(ASSIGN_COW_HYBRID, J_SITES, COWS, method_tag="hybrid", outage_geom=outage_geom)
    power_results = compute_power_coverage_from_hybrid(ASSIGN_POWER_HYBRID, FAILED_BTS, method_tag="hybrid", outage_geom=outage_geom)
