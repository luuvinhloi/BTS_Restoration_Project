# # src/optimization/solver_milp.py
# """
# Enhanced MILP Solver (Raster-aligned version)
#     - Đồng bộ vùng đo với raster (compute_population_coverage.py)
#     - Sau khi solver MILP chạy xong, tự động tính lại vùng phủ raster từ milp_assignments.csv
#     - Giữ nguyên pipeline, tương thích mô-đun mô phỏng hiện tại.
# """
#
# import numpy as np
# import pandas as pd
...
# from src.optimization.milp_model import build_milp_model
# from src.utils.io_utils import read_yaml, save_json
# from src.utils.geo_utils import compute_distance_matrix
#
# # import compute_population_coverage module (for raster-based coverage)
...
#     """Đọc dữ liệu đầu vào MILP"""
#     out_dir = Path(out_dir)
#     I_points = pd.read_csv(out_dir / "I_points.csv").to_dict(orient="records")
#     J_sites = pd.read_csv(out_dir / "J_sites.csv").to_dict(orient="records")
#
#     # Ép kiểu numeric cho COW dataset để tránh lỗi string
#     COWs_df = pd.read_csv(RAW_DIR / "cow_dataset.csv")
#     for col in ["coverage_radius_m", "cost_vnd", "speed_kmh", "endurance_hr"]:
#         if col in COWs_df.columns:
#             COWs_df[col] = pd.to_numeric(COWs_df[col], errors="coerce").fillna(0.0)
#     COWs = COWs_df.to_dict(orient="records")
#
#     return I_points, J_sites, COWs
#
# def solve_milp(config_path=CONFIG_PATH, out_dir=PROCESSED_DIR):
#     """Giải bài toán MILP và tính vùng phủ raster sau khi có kết quả"""
#     cfg = read_yaml(config_path)
#     I_points, J_sites, COWs = load_data(out_dir)
#
#     max_I = int(cfg.get("population_sample_limit", 2000))
#     max_J = int(cfg.get("candidate_max", 300))
#
#     if len(I_points) > max_I:
#         I_points = sorted(I_points, key=lambda d: d.get("pop", 0), reverse=True)[:max_I]
#     if len(J_sites) > max_J:
#         J_sites = J_sites[:max_J]
#
#     N, M, K = len(I_points), len(J_sites), len(COWs)
#     print(f"Using {N} demand points (I), {M} candidate sites (J), {K} COW units (K)")
#
#     # Build coverage and travel matrices
#     I_coords = [{"x": i.get("longitude"), "y": i.get("latitude")} for i in I_points]
#     J_coords = [{"x": j.get("longitude"), "y": j.get("latitude")} for j in J_sites]
#
#     dist_IJ = compute_distance_matrix(I_coords, J_coords, metric="haversine")
#     cover = np.zeros((N, M, K), dtype=np.uint8)
#     default_R = cfg.get("default_R", 3000)
#     for k, cow in enumerate(COWs):
#         R = cow.get("coverage_radius_m", default_R)
#         cover[:, :, k] = (dist_IJ <= R).astype(np.uint8)
#
#     # Travel times and costs
#     base_coords = [{"x": cow.get("lon"), "y": cow.get("lat")} for cow in COWs]
#     dist_KJ = compute_distance_matrix(base_coords, J_coords, metric="haversine")
#     travel_time = np.zeros((K, M))
#     travel_cost = np.zeros((K, M))
#
#     cost_per_km = cfg.get("cost_per_km_factor", 5000)  # VND/km
#     for k, cow in enumerate(COWs):
#         speed = max(float(cow.get("speed_kmh", 40)), 1.0)
#         travel_time[k, :] = dist_KJ[k, :] / (speed * 1000)  # giờ
#         travel_cost[k, :] = (dist_KJ[k, :] / 1000.0) * cost_per_km
#
#     # Model parameters
#     milp_cfg = cfg.get("milp", {})
#     params = {
#         "weights": milp_cfg.get("weights", {"w_time": 0.05, "w_dist": 0.05, "w_cost": 0.1}),
#         "M_max": milp_cfg.get("M_max", cfg.get("M_max", 36)),
#         "budget_max": float(milp_cfg.get("budget_max", cfg.get("budget_max", 5e8))),
#     }
#
#     print("Building optimized MILP model...")
#     model = build_milp_model(I_points, J_sites, COWs, cover, travel_time, travel_cost, params)
#
#     # Solver config
#     solver_cfg = milp_cfg.get("solver", {})
#     solver_name = solver_cfg.get("engine", "gurobi")
#     time_limit = solver_cfg.get("time_limit", 1000)
#
...
#     })
#
#     print(f"Solving MILP with {solver_name} (time limit = {time_limit}s)...")
#     result = solver.solve(model, tee=True)
#
#     # Extract results
#     x_sol = np.array([[pyo.value(model.x[k, j]) for j in model.J] for k in model.K])
#     z_sol = np.array([pyo.value(model.z[i]) for i in model.I])
#
#     chosen_pairs = [(k, j) for k in range(K) for j in range(M) if x_sol[k, j] > 0.5]
#     covered_points = [i for i in range(N) if z_sol[i] > 0.5]
#
#     covered_pop = sum(float(I_points[i]["pop"]) for i in covered_points)
#     total_pop = sum(float(I_points[i]["pop"]) for i in range(N))
#     coverage_pct = covered_pop / total_pop * 100 if total_pop > 0 else 0
#
#     total_fixed_cost = sum(float(COWs[k].get("cost_vnd", 0.0)) for (k, j) in chosen_pairs)
#     total_travel_cost = sum(float(travel_cost[k, j]) for (k, j) in chosen_pairs)
#     total_cost_all = total_fixed_cost + total_travel_cost
#     total_time = sum(float(travel_time[k, j]) for (k, j) in chosen_pairs)
#
#     chosen_site_ids = [
#         str(J_sites[j].get("site_id", f"J_{j:05d}")) for (_, j) in chosen_pairs if j < len(J_sites)
#     ]
#
#     # Save results to file
#     output_path = PROJECT_ROOT / "outputs" / "results"
#     output_path.mkdir(parents=True, exist_ok=True)
#
#     assign_list = []
#     for (k, j) in chosen_pairs:
#         site_id_val = J_sites[j].get("site_id", f"J_{j:05d}")
#         site_id_str = str(site_id_val)
#         assign_list.append({
#             "cow_id": str(COWs[k].get("cow_id", f"COW_{k + 1}")),
#             "type": str(COWs[k].get("type", "unknown")),
#             "base_id": str(COWs[k].get("base_id", None)),
#             "assigned_site_index": j + 1,
#             "assigned_site_id": site_id_str
#         })
#
#     assign_df = pd.DataFrame(assign_list)
#     assign_path = output_path / "milp_assignments.csv"
#     assign_df.to_csv(assign_path, index=False)
#
#     out = {
#         "num_COW_used": len(chosen_pairs),
#         "chosen_site_ids": chosen_site_ids,
#         "coverage_pct_on_I_points": coverage_pct,
#         "covered_pop_on_I_points": covered_pop,
#         "total_pop_on_I_points": total_pop,
#         "total_fixed_cost_vnd": total_fixed_cost,
#         "total_travel_cost_vnd": total_travel_cost,
#         "total_cost_all_vnd": total_cost_all,
#         "total_time_hours": total_time,
#     }
#
#     save_json(out, str(output_path / "milp_solution_summary.json"))
#     print(f"MILP solution saved to {output_path}/milp_solution_summary.json")
#     print(f"Coverage (I_points): {coverage_pct:.2f}% | Used {len(chosen_pairs)} COWs | Pop covered: {int(covered_pop)}")
#
#     # Compute raster-based coverage for consistency with GA-PSO
#     print("\n--- Computing raster-based population coverage for MILP ---")
#     raster_summary = main_compute_all(method="MILP")
#
#     # Merge raster-based metrics into MILP summary
#     out.update({
#         "raster_total_population": raster_summary.get("total_population", 0.0),
#         "raster_population_covered_by_active_bts": raster_summary.get("population_covered_by_active_bts", 0.0),
#         "raster_population_outage_due_failed_bts": raster_summary.get("population_outage_due_failed_bts", 0.0),
#         "raster_population_restored_by_cows": raster_summary.get("population_restored_by_cows", 0.0),
#         "raster_coverage_restored_percent_of_outage": raster_summary.get("coverage_restored_percent_of_outage", 0.0),
#         "raster_coverage_after_restoration_percent": raster_summary.get("coverage_after_restoration_percent", 0.0)
#     })
#
#     save_json(out, str(output_path / "milp_solution_summary.json"))
#     print(f"Raster-based coverage computed and merged into milp_solution_summary.json")
#
#     return out
#
#
