...
Runs lexicographic MILP as:
  Step 1: Maximize restored BTS coverage (COW locked).
  Step 2: Minimize deployment time T_max, with COW forced to cover all remaining uncovered demands.
  Step 3: Minimize total deployment cost, with BTS restoration and T_max fixed.
"""
...
def build_cover_and_travel_maps(data, params):
    ...
    # Build COW overlap constraints
    cow_overlap_pairs = []

    # dùng bán kính COW LỚN NHẤT để đảm bảo an toàn
    max_cow_radius = max(
        to_float(r["coverage_radius_m"]) for _, r in cows.iterrows()
    )

    site_ids = list(j_sites["site_id"].astype(str).values)
    site_pos = {
        str(r["site_id"]): (to_float(r["latitude"]), to_float(r["longitude"]))
        for _, r in j_sites.iterrows()
    }

    for i, j1 in enumerate(site_ids):
        lat1, lon1 = site_pos[j1]
        for j2 in site_ids[i + 1:]:
            lat2, lon2 = site_pos[j2]
            d_m = haversine_km(lat1, lon1, lat2, lon2) * 1000.0
            if d_m <= max_cow_radius:
                cow_overlap_pairs.append((j1, j2))
    ...
    preproc = {
        ...
        "cow_overlap_pairs": cow_overlap_pairs
    }
    return preproc
...
def build_milp_problem(preproc, data, params):
    ...
    # No overlapping COW coverage
    for (j1, j2) in preproc.get("cow_overlap_pairs", []):
        prob += (
                pulp.lpSum(x[k][j1] for k in cow_ids)
                + pulp.lpSum(x[k][j2] for k in cow_ids)
                <= 1
        ), f"no_cow_overlap_{j1}_{j2}"
...
def build_bts_coverage_expression(w_bts, j_ids, bts_ids, pop_map, priority_map):
    """
    Covered population ONLY by restored BTS
    """
    return pulp.lpSum(
        pop_map[j]
        * priority_map.get(j, 1.0)
        * pulp.lpSum(w_bts[bi][j] for bi in bts_ids)
        for j in j_ids
    )

# Lexicographic solve orchestration
def run_lexicographic_for_solver(data, preproc, params, solver_name="CBC", out_dir="./outputs/milp_runs_new"):
    """
    Runs lexicographic MILP as:
    ...
    # KHÓA TOÀN BỘ COW
    for k in var_dict1["cow_ids"]:
        for j in var_dict1["j_ids"]:
            prob1 += x[k][j] == 0, f"lock_cow_step1_{k}_{j}"

    priority_map = {
        str(r["site_id"]): to_float(r.get("priority_weight", 1.0))
        for _, r in j_sites.iterrows()
    }

    # Objective: maximize BTS coverage only
    bts_cov_expr = build_bts_coverage_expression(
        w_bts,
        var_dict1["j_ids"],
        var_dict1["bts_ids"],
        pop_map,
    ...
    prob1.setObjective(bts_cov_expr)

    print("Solving Step 1: maximize BTS coverage ONLY...")
    t0 = time.time()
    prob1.solve(solver)
    t1 = time.time()

    status1 = pulp.LpStatus[prob1.status]
    optimal_bts_covered_pop = pulp.value(bts_cov_expr)

    print(
        f" Step1 status={status1}, "
        f"BTS_covered_pop={optimal_bts_covered_pop:.2f}, "
        f"time={t1 - t0:.2f}s"
    )
    ...
    # Identify remaining uncovered demand points
    covered_by_bts = set()

    for bi in var_dict1["bts_ids"]:
        for j in var_dict1["j_ids"]:
            try:
                val = pulp.value(prob1.variablesDict()[f"w_bts_{bi}_{j}"])
            except KeyError:
                val = 0
            if val is not None and round(val) == 1:
                covered_by_bts.add(j)

    P_remain = [j for j in var_dict1["j_ids"] if j not in covered_by_bts]

    restored_bts = set()
    for bi in var_dict1["bts_ids"]:
        val = pulp.value(prob1.variablesDict()[f"u_{bi}"])
        if val is not None and round(val) == 1:
            restored_bts.add(bi)

    power_assign_step1 = [
        (g, bi)
        for g in power_ids for bi in bts_ids
        if pulp.value(prob1.variablesDict().get(f"z_{g}_{bi}", 0)) == 1
    ]

    print(f" Remaining uncovered demand points: {len(P_remain)}")

    # STEP 2: COW MUST COVER ALL REMAINING DEMANDS
    ...
    # Giữ nguyên BTS coverage từ Step 1
    u2 = var_dict2["u"]
    for bi in restored_bts:
        prob2 += u2[bi] == 1, f"freeze_u_step2_{bi}"

    # ÉP COW phủ toàn bộ P_remain
    for j in P_remain:
        prob2 += pulp.lpSum(y[j][jj] for jj in var_dict2["j_ids"]) == 1, f"cow_must_cover_{j}"

    # Objective: minimize deployment time
    prob2.sense = pulp.LpMinimize
    prob2.setObjective(T_max)

    z2 = var_dict2["z"]
    for (g, bi) in power_assign_step1:
        prob2 += z2[g][bi] == 1

    print("Solving Step 2: minimize deployment time (COW covers all remaining)...")
    t0 = time.time()
    prob2.solve(solver)
    t1 = time.time()

    status2 = pulp.LpStatus[prob2.status]
    optimal_T_max = pulp.value(T_max)

    print(
        f" Step2 status={status2}, "
        f"T_max={optimal_T_max:.4f}, "
        f"time={t1 - t0:.2f}s"
    )
    ...
    cow_assign_step2 = [
        (k, j)
        for k in cow_ids for j in j_ids
        if pulp.value(prob2.variablesDict().get(f"x_{k}_{j}", 0)) == 1
    ]
    ...
    # RE-ENFORCE: COW must cover all remaining demands
    for j in P_remain:
        prob3 += pulp.lpSum(y[j][jj] for jj in j_ids) == 1, f"cow_must_cover_step3_{j}"

    # Freeze BTS restoration decisions
    for bi in restored_bts:
        prob3 += u[bi] == 1, f"freeze_u_{bi}"

    x3 = var_dict3["x"]
    for (k, j) in cow_assign_step2:
        prob3 += x3[k][j] == 1, f"freeze_cow_{k}_{j}"
    ...
    res = {
        ...
        "optimal_covered_pop": optimal_bts_covered_pop,
...
# Main runner
def main_solve(config_params: dict, processed_data_dir: str, outputs_dir: str = None, solver_name: str = "CBC"):
    """
    ...
    if outputs_dir is None:
        outputs_dir = Path.cwd() / "outputs" / "milp_runs_new"
    else:
    ...
    if solver_name == "GUROBI":
        if not pulp.GUROBI_CMD().available():
            raise RuntimeError("GUROBI selected but not available or license missing.")
    elif solver_name == "CBC":
        pass
    else:
        raise ValueError("solver_name must be 'GUROBI' or 'CBC'")

    print(f"\n=== Running MILP with solver {solver_name} ===")

    res = run_lexicographic_for_solver(
        data,
        preproc,
        config_params,
...
if __name__ == "__main__":
    ...
    outdir = PROJECT_ROOT / "outputs" / "milp_runs_new"
