...
# =====================================================
# Preprocessing (GIỮ NGUYÊN LOGIC FILE CŨ)
# =====================================================
def build_cover_and_travel_maps(data, params):
    ...
    # ---------- power compatibility (GIỮ LOGIC CŨ) ----------
...
# =====================================================
def solve_lexicographic_pulp(data, preproc, params, solver_name="CBC", out_dir="./outputs/milp_runs"):
    ...
    priority_map = {str(r["site_id"]): to_float(r.get("priority_weight", 0.0)) for _, r in j_sites.iterrows()}
    ...
    cov1 = build_coverage_expression_pulp(v1["y"], v1["w_bts"], v1["j_ids"], v1["bts_ids"], pop_map)

    def cow_priority_expr(x, cow_ids, j_ids, priority_map):
        return pulp.lpSum(priority_map[j] * pulp.lpSum(x[k][j] for k in cow_ids) for j in j_ids)

    EPS_PRIORITY = 1e-4
    prob1.sense = pulp.LpMaximize
    prob1.setObjective(cov1 + EPS_PRIORITY * cow_priority_expr(
        v1["x"], v1["cow_ids"], v1["j_ids"], priority_map
    ))
    ...
    # =========================
    # STEP 2 — Min T_max (fix coverage)
    # =========================
    prob2, v2 = build_milp_problem_pulp(preproc, data, params)
    cov2 = build_coverage_expression_pulp(v2["y"], v2["w_bts"], v2["j_ids"], v2["bts_ids"], pop_map)
    ...
    # =========================
    # STEP 3 — Min total cost (fix coverage & T_max)
    # =========================
    prob3, v3 = build_milp_problem_pulp(preproc, data, params)
    cov3 = build_coverage_expression_pulp(v3["y"], v3["w_bts"], v3["j_ids"], v3["bts_ids"], pop_map)
    ...
    # =========================
    # Extract solution (GIỐNG FILE CŨ)
    # =========================
    assignments_cow = []
    for k in v3["cow_ids"]:
        for j in v3["j_ids"]:
    ...
    for (k, j) in assignments_cow:
        total_fixed_cost += to_float(cows[cows["cow_id"] == k].iloc[0].get("cost_vnd", 0.0))
        total_travel_cost += to_float(cow_travel_map.get((k, j), {}).get("travel_cost_vnd", 0.0))

    for (g, b) in assignments_power:
        total_fixed_cost += to_float(power_lookup.get(g, {}).get("cost_vnd_24h", 0.0))
        total_travel_cost += to_float(power_travel_map.get((g, b), {}).get("total_cost_vnd", 0.0))

    total_cost_all = total_fixed_cost + total_travel_cost
    T_max_final = pulp.value(v3["T_max"])

    # =========================
    # Save outputs (CSV/JSON) — GIỐNG FILE CŨ
    # =========================
    ...
    df_cow["travel_cost_vnd"] = df_cow.apply(
        lambda r: to_float(cow_travel_map.get((r["cow_id"], r["site_id"]), {}).get("travel_cost_vnd", 0.0)), axis=1
    )
    df_cow["travel_time_hr"] = df_cow.apply(
        lambda r: to_float(cow_travel_map.get((r["cow_id"], r["site_id"]), {}).get("travel_time_hr", 0.0)), axis=1
    )
    ...
    df_power["travel_cost_vnd"] = df_power.apply(
        lambda r: to_float(power_travel_map.get((r["power_id"], r["bts_id"]), {}).get("total_cost_vnd", 0.0)), axis=1
    )
    ...
    df_power["travel_time_hr"] = df_power.apply(
        lambda r: to_float(power_travel_map.get((r["power_id"], r["bts_id"]), {}).get("total_time_hr", 0.0)), axis=1
    )
...
# =====================================================
def solve_lexicographic_pyomo(data, preproc, params, solver_name="GUROBI", out_dir="./outputs/milp_runs"):
    """
    Run lexicographic MILP using Pyomo + GUROBI
    BUT return results & write outputs identical to PuLP version.
    """
    ...
    cov_expr1 = (
        ...
        + sum(pop_map[j] * sum(model1.w_bts[b, j] for b in model1.B) for j in model1.J)
    )
    priority_expr = sum(
    ...
    EPS_PRIORITY = 1e-4
    model1.Obj = pyo.Objective(expr=cov_expr1 + EPS_PRIORITY * priority_expr, sense=pyo.maximize)
    ...
    cov_expr2 = (
        ...
        + sum(pop_map[j] * sum(model2.w_bts[b, j] for b in model2.B) for j in model2.J)
    )
    ...
    cov_expr3 = (
        ...
        + sum(pop_map[j] * sum(model3.w_bts[b, j] for b in model3.B) for j in model3.J)
    )
    ...
    # =====================================================
    # 🔁 TRÍCH NGHIỆM PYOMO → DÙNG CHUNG HÀM XUẤT (PU LP)
    # =====================================================
    # Convert Pyomo solution into PuLP-like structures
    # by creating a fake var_dict with .value access
    ...
    for (k, j) in assignments_cow:
        total_fixed_cost += to_float(data["cows"][data["cows"]["cow_id"] == k].iloc[0].get("cost_vnd", 0.0))
        total_travel_cost += to_float(cow_travel.get((k, j), {}).get("travel_cost_vnd", 0.0))
    ...
    # =====================================================
    # Save outputs — IDENTICAL TO PuLP
    # =====================================================
    ...
    df_cow["travel_cost_vnd"] = df_cow.apply(
        lambda r: to_float(cow_travel.get((r["cow_id"], r["site_id"]), {}).get("travel_cost_vnd", 0.0)), axis=1
    )
    df_cow["travel_time_hr"] = df_cow.apply(
        lambda r: to_float(cow_travel.get((r["cow_id"], r["site_id"]), {}).get("travel_time_hr", 0.0)), axis=1
    )
    df_cow["setup_time_h"] = float(params.get("default_setup_time_h", 0.5))
    df_cow["deployment_time_hr"] = df_cow["travel_time_hr"] + df_cow["setup_time_h"]
    df_cow.to_csv(out_run / f"assignments_cow_{solver_name}.csv", index=False)
    ...
    df_power["travel_cost_vnd"] = df_power.apply(
        lambda r: to_float(power_travel.get((r["power_id"], r["bts_id"]), {}).get("total_cost_vnd", 0.0)), axis=1
    )
    df_power["operating_cost_vnd_24h"] = df_power["cost_vnd_24h"]
    df_power["total_deployment_cost_vnd"] = df_power["travel_cost_vnd"] + df_power["operating_cost_vnd_24h"]
    df_power["travel_time_hr"] = df_power.apply(
        lambda r: to_float(power_travel.get((r["power_id"], r["bts_id"]), {}).get("total_time_hr", 0.0)), axis=1
    )
    df_power.to_csv(out_run / f"assignments_power_{solver_name}.csv", index=False)
...
# =====================================================
def main_solve(config_params: dict, processed_data_dir: str, outputs_dir: str = None, solver_backend="AUTO"):
