...
"""
Feature Extraction (refactored, flood-aware, upgraded to 'final' style)

Key changes compared to original optimize_B:
- Full refactor following feature_extraction_final.py design:
  * Method A: remove population inside union(active_bts + failed_bts with status='power_outage')
  * Only failed_bts with status == 'failed' are used to generate I (hard failures)
  * J candidate generation filters out vector water and flood raster areas (flood_depth >= threshold)
  * Multi-scale jitter, road sampling, global grid sampling for robust J pool
  * Improved selection (balanced greedy + farthest point sampling)
- Outputs same-named files as previous pipeline for compatibility:
    I_points_B.csv, J_sites_B.csv, cover.npy
- Default flood tif path: data/processed/flood/flood_depth_combined_B_clean.tif

Author: automated upgrade for user request
"""
...
# Local util imports (safe fallback)
try:
...
# default flood tif location requested by the user
FLOOD_TIF_DEFAULT = DATA_DIR / "processed" / "flood" / "flood_depth_combined_B_clean.tif"
...
def main(config: dict, out_dir: str or Path):
            ...
            if not I_df.empty:
                ...
                for k, idxn in enumerate(nearest):
                    try:
                        ...
                        pw.append(float(rec.get("priority_weight",1.0)))
                        pc.append(str(rec.get("priority_category","normal")))
                        pops.append(0.0)
                    except Exception:
    ...
    # ----- Save outputs (use original B filenames for pipeline compatibility) -----
    ...
    print(f"[Feature Extraction - UPGRADED] Saved {len(I_out)} I_points_B, {len(J_out)} J_sites_B, and cover.npy to {out_dir}")
    return {"I_points": out_dir / "I_points_B.csv",
            "J_sites": out_dir / "J_sites_B.csv",
